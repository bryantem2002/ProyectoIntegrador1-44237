...
function showEmailForm() {
  ...
  setTimeout(() => {
    ...
    document.getElementById('email-form').addEventListener('submit', async function(event) {
      ...
      try {
        const response = await fetch(`https://facipago.space/api/usuarios/correo/${encodeURIComponent(emailInput.value)}`, {
          method: 'GET',
          headers: {
...
function showPinForm() {
  ...
  setTimeout(() => {
    ...
    document.getElementById('login-button').addEventListener('click', async function() {
      ...
      try {
        const response = await fetch(`https://facipago.space/api/usuarios/login?correo=${encodeURIComponent(userData.email)}&contraseña=${encodeURIComponent(userData.pin)}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-API-Key': 'miClaveSecreta123'
