document.addEventListener('DOMContentLoaded', () => {
    ...
    async function validateAndShowTransferDetails(accountNumber) {
        ...
        try {
            ...
            const response = await fetch('/api/usuarios/cuenta/' + accountNumber, {
                method: 'GET',
    ...
    async function loadTransferHistory() {
        ...
        try {
            ...
            const response = await fetch('https://facipago.space/api/historial/combinado', {
                method: 'GET',
                headers: { 'Accept': 'application/json' }
            });
            if (!response.ok) {
...
async function loadUserData() {
    ...
    try {
        const userResponse = await fetch('/api/usuarios/datos', {
            method: 'GET',
            headers: { 'Accept': 'application/json' }
        });
        if (!userResponse.ok) {
        ...
        const accountResponse = await fetch('/api/usuarios/cuenta', {
            method: 'GET',
            headers: { 'Accept': 'application/json' }
        });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (confirmTransferButton) {
        confirmTransferButton.addEventListener('click', async () => {
            ...
            try {
                ...
                const url = '/api/transferencias?origen=' + encodeURIComponent(userAccountData.numeroCuenta) + '&destino=' + encodeURIComponent(recipientData.numero_cuenta) + '&monto=' + encodeURIComponent(amount.toFixed(2)) + '&mensaje=' + encodeURIComponent(message);
    ...
    async function performRecharge() {
        ...
        try {
            ...
            const url = `https://facipago.space/api/recargas?monto=${encodeURIComponent(amount.toFixed(2))}&metodo=${encodeURIComponent(method)}`;
            const response = await fetch(url, {
                method: 'POST',
                headers: { 'Accept': 'application/json' }
            });
