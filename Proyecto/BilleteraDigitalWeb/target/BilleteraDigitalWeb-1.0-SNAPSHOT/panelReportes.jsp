...
<%@page import="com.mycompany.billeteradigitalweb.model.Recarga"%>
<html lang="es">
<head>
    <meta charset="UTF-8">
...
<!-- Formulario para filtrar recargas -->
<div class="bg-white shadow rounded-2xl p-6 mb-6">
  <h3 class="text-lg font-semibold text-gray-600 mb-4">Cantidad de Recargas</h3>
  <form id="filtroRecargas" class="flex flex-col md:flex-row md:items-center md:space-x-4 space-y-4 md:space-y-0">
    <div class="flex flex-col">
      <label for="fechaDesde" class="text-gray-500 mb-1 font-medium">Desde</label>
      <input type="date" id="fechaDesde" name="fechaDesde" class="border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#2BB15D]" />
    </div>
    <div class="flex flex-col">
      <label for="fechaHasta" class="text-gray-500 mb-1 font-medium">Hasta</label>
      <input type="date" id="fechaHasta" name="fechaHasta" class="border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#2BB15D]" />
    </div>
    <button type="submit" class="bg-[#2BB15D] text-white px-6 py-2 rounded-xl hover:bg-green-600 transition font-semibold">
      Buscar
    </button>
    <button type="button" id="btnDescargarPdf" class="bg-gray-200 text-gray-700 px-6 py-2 rounded-xl hover:bg-gray-300 transition font-semibold flex items-center space-x-2">
...
<div id="tablaRecargas" class="bg-white shadow rounded-2xl p-6 mt-6"></div>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Regex para validar fecha YYYY-MM-DD
    const fechaRegex = /^\d{4}-\d{2}-\d{2}$/;

    document.getElementById('filtroRecargas').addEventListener('submit', function(event) {
      ...
      const url = `${basePath}/BilleteraDigitalWeb/api/recargas/filtrar-por-fecha?fechaDesde=${fechaDesde}&fechaHasta=${fechaHasta}`;

      fetch(url)
        .then(async response => {
          if (!response.ok) {
            let errorMsg = 'Error al obtener datos';
            try {
        ...
        .then(data => {
          mostrarRecargasEnTabla(data);
        })
        .catch(error => {
          alert('No se pudieron obtener las recargas: ' + error.message);
        });
    });
    ...
    document.getElementById('btnDescargarPdf').addEventListener('click', function() {
      ...
      if (!fechaDesde || !fechaHasta) {
        alert('Por favor, ingresa ambas fechas para descargar el PDF');
        return;
      }
      ...
      const url = `${basePath}/BilleteraDigitalWeb/api/recargas/pdf?fechaDesde=2025-06-03&fechaHasta=2025-06-06`;
window.open(url, '_blank');
    });

    function mostrarRecargasEnTabla(recargas) {
      const contenedor = document.getElementById('tablaRecargas');
      contenedor.innerHTML = '';

      if (!recargas || recargas.length === 0) {
        contenedor.innerHTML = '<p class="text-gray-500">No se encontraron recargas para el rango seleccionado.</p>';
        return;
      }

      const tabla = document.createElement('table');
      tabla.className = 'min-w-full table-auto border-collapse border border-gray-300';

      const thead = document.createElement('thead');
      thead.innerHTML = `
        <tr class="bg-gray-100">
          <th class="border border-gray-300 px-4 py-2 text-left">ID Recarga</th>
          <th class="border border-gray-300 px-4 py-2 text-left">Fecha</th>
          <th class="border border-gray-300 px-4 py-2 text-left">Cuenta</th>
          <th class="border border-gray-300 px-4 py-2 text-left">Método</th>
      ...
      recargas.forEach(recarga => {
        ...
        tr.innerHTML = `
          <td class="border border-gray-300 px-4 py-2">${recarga.idRecarga}</td>
          <td class="border border-gray-300 px-4 py-2">${recarga.fecha}</td>
          <td class="border border-gray-300 px-4 py-2">${recarga.idCuenta}</td>
          <td class="border border-gray-300 px-4 py-2">${recarga.metodo}</td>
          <td class="border border-gray-300 px-4 py-2 text-right">${recarga.monto.toFixed(2)}</td>
        `;
